        ...
        <el-card class="box-card">
            ...
            <el-table :data="QTLData.SnpEpistasis" style="width: 100%;" empty-text="No results found for your query">
                  ...
                  <template v-for="column in tableColumns">
                      ...
                      <el-table-column 
                          v-if="column.prop !== 'SNP' && column.prop !== 'SNP_epi' && column.prop !== 'coexpression' && column.prop !== 'textmining_transferred' && column.prop !== 'combined_score'" 
                          :key="column.prop" 
                          :prop="column.prop" 
                          :label="column.label" 
                          :width="column.width" 
                          ...
                          show-overflow-tooltip>
                          <template slot-scope="scope">
                              <!-- <span v-if="column.prop === 'BETA'">
                                  {{ scope.row[column.prop] !== null ? parseFloat(scope.row[column.prop]).toFixed(2) : '' }}
                              </span>
                              <span v-else-if="column.prop === 'P'">
                                  {{ scope.row[column.prop] !== null ? parseFloat(scope.row[column.prop]).toExponential(2) : '' }}
                              </span>
                              <span v-else-if="column.prop === 'OR'">
                                  {{ scope.row[column.prop] !== null ? parseFloat(scope.row[column.prop]).toFixed(2) : '' }}
                              </span>
                              <span v-else-if="column.prop === 'saige_p'">
                                  {{ scope.row[column.prop] !== null ? parseFloat(scope.row[column.prop]).toExponential(2) : '' }}
                              </span> -->
                              <span v-else-if="['FuncrefGene', 'GenerefGene', 'ExonicFuncrefGene', 'AAChangerefGene', 'FuncrefGene1', 'GenerefGene1', 'ExonicFuncrefGene1', 'AAChangerefGene1', 'FuncrefGene2', 'GenerefGene2', 'ExonicFuncrefGene2', 'AAChangerefGene2'].includes(column.prop)">
                                  <span v-html="scope.row[column.prop] === 'NA' ? '&#8203;<i class=\'el-icon-minus\' style=\'color: #FA8072;\'></i>' : scope.row[column.prop]"></span>
                              </span>
                              <span v-else>
                                  {{ scope.row[column.prop] }}
                              </span>
                      ...
                      <el-table-column 
                          v-if="column.prop === 'coexpression' || column.prop === 'textmining_transferred' || column.prop === 'combined_score'" 
                          :key="column.prop" 
                          :prop="column.prop" 
                          :label="column.label" 
                          :width="column.width" 
                          ...
                          show-overflow-tooltip>
                          <template slot-scope="scope">
                              <span v-if="scope.row[column.prop] === 'NA'">
    ...
    displayDataBasedOnModel() {
        if (this.selectQueryType === 'SNP') {
            const displayColumns = [
                ...
                { prop: "AAChangerefGene", label: "AAChange.refGene", width: 160 }
            ];

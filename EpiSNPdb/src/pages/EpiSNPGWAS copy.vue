            ...
            <div>
                <el-row :gutter="20" type="flex" justify="center">
                    ...
                    <el-col :span="5" :offset="0" v-if="selectQueryType === 'SNP'">
                        ...
                        <span :class="{ 'text-black': !isClicked1, 'text-orange': isClicked1 }" @click="handleClickAndSetQueryRsid1" style="font-size: 17px;">e.g., rs17632542</span>
                    </el-col>
                    ...
                    <el-col :span="5" :offset="0" v-if="selectQueryType === 'Chromosome Range'">
                        ...
                        <el-input v-model="queryRsid1" placeholder="Chromosome Range" class="place-input" style="width: 100%;"></el-input>
                        <span :class="{ 'text-black': !isClicked2, 'text-orange': isClicked2 }" 
                        @click="handleClickAndSetQueryRsid2" style="font-size: 17px;">e.g., chr1:1003629-1022154</span>
                    </el-col>
                    <!-- P-value filter select -->
                    <el-col :span="5" :offset="0">
                        <span class="input-label">P-value Select:</span>
                        <el-select v-model="pValueThreshold" placeholder="P-value" class="place-input p-value-select" style="width: 100%;">
        ...
        <el-card class="box-card">
            ...
            >
                ...
                <el-table-column
                    ...
                    show-overflow-tooltip>
                    <template v-slot="scope">
                        <a-icon type="bar-chart" theme="outlined" @click="fetchPdf(scope.row.Cancertype, scope.row.SNP)" style="font-size: 2.1em; cursor: pointer; color: blueviolet;"/>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
        ...
        handleClickAndSetQueryRsid2() {
            ...
            this.queryRsid1 = 'chr1:1003629-1022154';
        ...
        formatQueryInput(input) {
            // 去掉前后和中间的空格
            input = input.replace(/\s+/g, '');

            // Check if the input starts with 'rs' (case insensitive)
            if (/^rs/i.test(input)) {
                return input.replace(/^rs/i, 'rs');
            }
            // Check if the input starts with 'Affx-' (case insensitive)
            if (/^Affx/i.test(input)) {
                return input.replace(/^Affx/i, 'Affx-');
            }
            // Return the input as is if no changes are needed
            return input;
        },
        submitForm() {
            this.loading = true;
            let params = {
                queryType: this.selectQueryType || 'SNP',
            ...
            if (this.selectQueryType === 'SNP') {
                params.snp = this.queryRsid1;
                params.cancerType = this.selectCancerType
            } else if (this.selectQueryType === 'Chromosome Range') {
                params.chromosomeRange = this.queryRsid1
            }
        ...
        downloadCSV() {
            const headers = ['Cancer', 'SNP', 'P_value'];

      ...
      <el-row class="custom-row" :style="{ marginBottom: '15px' }">
        <el-col :span="24">
          <p class="card-description" style="font-size: 18px; line-height: 1.7; font-family: 'Helvetica';">
            This module provides 202,032 epistasis with a P-value less than 1e-5 across 16 cancer types, identified through the boost, epistasis, and joint methods.
            
            This module provides <span style="color: blue; font-weight: bold;">202,032</span> epistasis with a P-value less than 1e-5 across 16 cancer types.
          </p>
        </el-col>
      </el-row>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="QTLData.SnpEpistasis" style="width: 100%;" empty-text="No SNP epistasis results found for your query">
        ...
        <el-table-column prop="P_epistasis" label="P_epistasis" width="146" sortable align="center" show-overflow-tooltip class="custom-column-header">
          ...
          <template v-slot="scope">
            {{ scope.row.P_epistasis !== undefined && scope.row.P_epistasis != 100 ? scope.row.P_epistasis.toExponential(2) : '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="P-saige" width="117" sortable align="center" class="custom-column-header">
          ...
          <template v-slot="scope">
            {{ scope.row.P_saige !== undefined ? (scope.row.P_saige === 0 ? '0' : scope.row.P_saige.toExponential(2)) : '' }}
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      isClicked2: false,
      isClicked3: false,
      queryRsid1: 'rs6687430_rs17781327',
      queryRsid2: '',
      flag: 1,
      currentPage: 1,
      perPage: 8,
      rowHeight: 60,
      defaultQueryType: 'SNP epistasis',
      defaultCancerType: 'Melanoma',
  ...
  mounted() {
    this.initData();
    this.selectQueryType = this.defaultQueryType; // 设置默认癌症类型
    this.selectCancerType = this.defaultCancerType; // 设置默认癌症类型
    this.pValueThreshold = this.defaultPValueThreshold; // 设置默认 P-value 阈值

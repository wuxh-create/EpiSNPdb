<template>
    <div>
        <div @click="closeAlert">
            <!-- 动态效果的容器 -->
            <div id="container" :style="{ minHeight: '350px', maxHeight: '10000px', height: 'auto' }"></div>
            <el-card class="box-card" style="margin-top: 20px; width: 1140px;padding: 0;">
                ...
                <el-row style="border-radius: 0px 0px 5px 5px; border: 1px solid #D6EADF;margin-bottom: 0px;">
                    <el-table
                        ...
                        style="width: 100%;margin-top: 0;font-size: 15px;"
                        max-height="500"
                        stripe
                ...
                <el-row style="border-radius: 0px 0px 5px 5px; border: 1px solid #D6EADF;margin-bottom: 0px;">
                    <el-table
                        ...
                        <el-table-column
                            ...
                            label="SNP"
                            width="140"
                            align="center" class="custom-column-header">
                        ...
                        <el-table-column label="Position" width="160" align="center" class="custom-column-header">
                            <template v-slot="scope">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                snpGWASData: [],
                snpEpiTableData: [], // 用于表格展示的SNP Epistasis数据
                snpEpiNetworkData: [], // 用于绘制网络图的SNP Epistasis数据
                loading: false,
                recordNumber: 0,
                epiRecordNumber: 0, // 新增：用于Epistasis结果的总数
                pdfDialogVisible: false, // 控制PDF对话框的显示
                imageUrl: '', // 存储图片URL
                imageDialogVisible: false, // 控制图片对话框的显示
                downloadClicked: false,  // 添加用于跟踪下载按钮点击状态的属性
                currentSNP: '', // 当前查询的SNP
                graphHeight: 'auto',
                currentGWASPage: 1, // 新增：GWAS当前页码
                currentEpiPage: 1, // 新增：Epistasis当前页码
                cancerTypes: [
        ...
        methods: {
            ...
            // 新增：Epistasis分页变化处理
            currentEpiChange(page) {
                const queryID = this.$route.params.queryID;
                this.loading = true;
                this.currentEpiPage = page;

                axios.get('api/quickSearchSnp', {
                    params: {
                        'queryID': queryID,
                        'page': page,
                        'perPage': 8,
                        'type': 'epistasis' // 标识查询类型
                    }
                }).then(
                    response => {
            ...
            // 新增：GWAS分页变化处理
            currentGWASChange(page) {
                const queryID = this.$route.params.queryID;
                this.loading = true;
                this.currentGWASPage = page;
            ...
            initializeHighcharts() {
                ...
                this.graphHeight = `${Math.min(Math.max(350, nodes.length * 10), 1000)}px`;
            ...
            // 兼容原有的currentChange方法
            currentChange(page) {
                this.currentGWASChange(page);
            },
            handleClose() {
        ...
        mounted() {
            ...
            axios.get('api/quickSearchSnp', {
                ...
                response => {
                        ...
                        this.snpEpiNetworkData = response.data.SNPEpiNetwork || [], 
                        this.recordNumber = response.data.recordsNumber,
                        this.epiRecordNumber = response.data.epiRecordsNumber || 0 // 新增
                    this.initializeHighcharts(); 
                },
                error => {

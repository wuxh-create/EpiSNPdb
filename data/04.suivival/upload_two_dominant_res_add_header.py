...
# 指明数据的绝对路径和文件名
input_file_path = '/home/wuxh/SSIdb3/data/04.suivival/two_dominant_res_add_header_filter_p_0.05_remove_na'
collection_name = 'TwoDominantSurvival'  # 在 MongoDB 中显示的表格名称
# 定义导入功能
    ...
    with open(input_file_path, 'r') as file:
        ...
        for row in reader:
            ...
            document = {
                ...
                'OS_Median_AABB': float(row['Median_OS_AABB']),
                'OS_Median_AABb_and_AAbb': float(row['Median_OS_AABb_and_AAbb']),
                'OS_Median_AaBB_and_aaBB': float(row['Median_OS_AaBB_and_aaBB']),
                'OS_Median_AaBb_Aabb_aaBb_and_aabb': float(row['Median_OS_AaBb_Aabb_aaBb_and_aabb']),
            }

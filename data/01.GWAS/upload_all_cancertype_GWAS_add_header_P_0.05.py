...
# 指明数据的绝对路径和文件名
input_file_path = '/home/wuxh/SSIdb3/data/01.GWAS/all_cancertype_gwas_epiSNP_add_header_add_example_tag'
collection_name = 'epiSNPGWASres'  # 在 MongoDB 中显示的表格名称
...
def import_data(input_file_path):
    ...
    with open(input_file_path, 'r') as file:
        ...
        for row in reader:
            ...
            row['Alt_Freqs'] = round(float(row['Alt_Freqs']), 4)
            row['A1'] = str(row['A1'])
            row['A2'] = str(row['A2'])
            row['N'] = str(row['N'])
            row['AF1'] = round(float(row['AF1']), 4)
            row['BETA'] = round(float(row['BETA']), 4)
            row['P'] = float(row['P'])
            row['case'] = str(row['case'])
            row['control'] = str(row['control'])
...
db[collection_name].create_index([('Pos', 1)])
db[collection_name].create_index([('POS', 1)])
